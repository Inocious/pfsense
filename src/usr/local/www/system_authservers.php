...
if ($_REQUEST['ajax']) {
	...
	$authcfg['ldap_clientcert'] = $_REQUEST['clientcert'];
...
if ($act == "edit") {
	if (isset($id) && $a_server[$id]) {
		...
		if ($pconfig['type'] == "ldap") {
			...
			$pconfig['ldap_mtls_enabled'] = $a_server[$id]['ldap_mtls_enabled'];
			$pconfig['ldap_clientcert'] = $a_server[$id]['ldap_clientcert'];
...
$section->addInput(new Form_Checkbox(
 ...
 foreach (config_get_path('cert', []) as $cert) {
	if (!preg_match('/^GUI default \(.+\)$/', $cert['descr'])) {
        $ldapClientCertRef[$cert['refid']] = $cert['descr']; // Skip if $cert['descr'] = 'GUI default (*)'
    }
 }
 ...
 $group->add(new Form_Select(
	...
	$ldapClientCertRef
 ))->setHelp('This Cert is used to authenticate to the LDAP server certificate when '.
	'\'mTLS\' is active. '.
	'This Certificate must be generated by the same CA presented by the LDAP server.');
...
events.push(function() {
	...
	function select_clicked() {
		if (document.getElementById("ldap_port").value == '' ||
			...
			document.getElementById("ldap_clientcert").value == '' ||
...
<?php if (count(config_get_path('cert', [])) > 0): ?>
                       clientcert = $('#ldap_clientcert').val();
<?php else: ?>
                       clientcert = '';
<?php endif; ?>
/*
...
<?php endif; ?>
	...
	hideClass('mtls', !$('#ldap_mtls_enabled').prop('checked'));
